...
const styleGuides = [
  ...
  {
    id: "cleanbite", category: "wellness", name: "CleanBite", style: "荧光食品扫描", liveDemo: "./demo/cleanbite-scanner/index.html", poster: "./demo/cleanbite-scanner/mobile-preview.png", preview: "#d4ff70", reference: "荧光青柠展板上的双手机，把会员选择和食品营养拆解放进强对比的黑白层级", referenceImage: "./demo/cleanbite-scanner/assets/cleanbite-effect-board.png", prompt: "食品扫描 App 视觉参考：荧光青柠绿色外部展板、黑色 iOS 机身、白与浅绿页面、黑色信息面板和圆形营养构成图；产品图只承担真实食品包装与苹果主体，所有价格、权益、百分比、状态栏、返回、分享、收藏、tab、按钮和图表标签均由代码呈现；不把新的 UI glyph 或可交互文字生成进图片。",
  ...
  {
    id: "plate-play", category: "commerce", name: "Plate Play", style: "高彩插画食谱", liveDemo: "./demo/plate-play/index.html", poster: "./demo/plate-play/mobile-preview.png", effectImage: "./demo/plate-play/assets/reference-overview.png", preview: "#f2b9d1", reference: "浅粉展板、荧光黄绿手机界面与番茄红厨师插画，像一本活泼的厨房图画书", referenceImage: "./demo/plate-play/assets/reference-overview.png", prompt: "高彩插画食谱 App 视觉参考：浅粉色展板，荧光黄绿色与浅粉色手机界面，番茄红、杏橙、淡紫和天蓝作为拼贴式强调色；用粗黑线描与扁平色块绘制一位正在搅拌面糊的厨师和厨房器具，插画无文字、无 logo、无系统状态栏、无按钮、无导航、无 UI glyph。所有标题、食谱名称、营养信息、按钮、标签、状态栏与底部导航均由代码呈现。",
  ...
  {
    id: "itinerary", category: "travel", name: "Aegean", style: "海岸行程编排", liveDemo: "./demo/lisbon-itinerary/index.html", poster: "./demo/lisbon-itinerary/mobile-preview.png", preview: "#dfe5e2", reference: "纸本行程表的从容层级，叠加轻量的路线与天气信息", referenceImage: "./demo/lisbon-itinerary/mobile-preview.png", prompt: "海岸旅行行程 App 视觉参考：有纸张感的浅灰白底，深青与珊瑚红作为少量功能强调，路线照片有明确留白，时间线清晰安静；把信息安排成一天的节奏，不做旅游平台式的密集卡片与促销标签。",
  ...
  {
    id: "journal", category: "travel", name: "Lumen", style: "旷野日记探索", liveDemo: "./demo/lumen-journal/index.html", poster: "./demo/lumen-journal/screenshot-app-frame.png", preview: "#d5e4e7", reference: "自然摄影的辽阔感，配合日记式短句和少量探索入口", referenceImage: "./demo/lumen-journal/screenshot-app-frame.png", prompt: "旅行日记探索 App 视觉参考：钴蓝水面与珊瑚红主体的摄影主视觉，文字像现场笔记一样简短，深色蓝色 UI chrome 克制存在；保留很强的单图情绪，不用大量旅游缩略图、评分和商业化标签。",
  ...
  {
    id: "notebook", category: "creative", name: "Marble Note", style: "手账式创意工作台", liveDemo: "./demo/marble-note/index.html", poster: "./demo/marble-note/screenshots/marble-note-mobile.png", preview: "#f0eac6", reference: "手绘线条、彩色贴纸和真实纸张节奏，让工作台更像创作空间", referenceImage: "./demo/marble-note/screenshots/marble-note-mobile.png", prompt: "创意笔记 App 视觉参考：柔和纸色底、手绘线稿、糖果色标签和文件夹形状，信息结构依然清楚可读；让插画性细节限定在封面与少量空白区域，不用每张卡片都铺满 emoji、渐变和厚重阴影。",
  ...
  {
    id: "softly-reflections", category: "wellness", name: "SOFTLY", style: "柔和反思与情绪签到", liveDemo: "./demo/softly-reflections/index.html", poster: "./demo/softly-reflections/assets/softly-effect-board.png", preview: "#e5e0fb", reference: "深靛网格展板上的三屏情绪产品，用纸卡、编辑式衬线与淡紫 3D 角色组织一次安静的自我对话", referenceImage: "./demo/softly-reflections/assets/reference-overview.png", prompt: "情绪反思 App 视觉参考：先根据参考图生成完整三屏 SOFTLY 效果图并审查，再以生成效果图作为 UI 拆分依据；深靛网格展板、珍珠白与暖象牙页面、淡紫角色、黄油黄纸卡和少量薄荷细节。角色与头像使用独立无文字 image2 位图，所有问题、按钮、状态栏、轮播箭头、标签、波形、完成环和底部导航由代码呈现；不在图片中生成文字、logo、水印、手机或 UI glyph。",
...
function renderDemoGallery() {
  ...
  gallery.innerHTML = guides.map((guide) => {
    ...
    const openMode = guide.defaultPreviewMode || (guide.liveDemo ? "live" : mediaMode);
    ...
    <article class="demo-card" data-case-id="${guide.id}">
      ...
      <div class="demo-card-body">
        ...
        <div class="demo-card-footer"><small title="本地参考图：${guide.referenceImage}">本地参考图 · ${guide.reference}</small><div class="demo-card-actions">${guide.liveDemo ? `<button class="style-details-button" type="button" data-preview-id="${guide.id}" data-preview-mode="live">可点击</button>` : guide.video ? `<button class="style-details-button" type="button" data-preview-id="${guide.id}" data-preview-mode="video">视频</button>` : ""}<button class="style-details-button" type="button" data-style-details="${guide.id}">查看要点</button><button class="copy-style-button" type="button" data-copy-style="${guide.id}" title="复制图片与提示词配置">复制配置</button></div></div>
      </div>
    </article>
  `;
  }).join("");
...
function openPreview(id, mode = "auto") {
  ...
  const modes = getPreviewModes(guide);
  const initialMode = mode === "auto" ? (guide.defaultPreviewMode || (guide.liveDemo ? "live" : (guide.video ? "video" : "image"))) : (modes.includes(mode) ? mode : modes[0]);
