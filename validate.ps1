...
Assert-True ($skill.Contains("按角色而不是名称")) "SKILL.md should classify image assets by UI role, not only by filename"
...
Assert-True ($auditScript.Contains("VISUAL_ASSET_ROLE_RE")) "ui_output_audit.mjs should recognize product/cutout visual asset roles"
Assert-True ($auditScript.Contains("VISUAL_CONTEXT_RE")) "ui_output_audit.mjs should keep weak visual contexts separate from explicit UI glyph names"
Assert-True ($auditScript.Contains("PHYSICAL_OBJECT_NAME_RE")) "ui_output_audit.mjs should recognize physical object names as possible visual assets"
