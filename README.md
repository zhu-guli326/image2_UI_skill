# 🎨 Image to UI Skill

<div align="center">

```ascii
                                                                                             
                                                                                             
                         ██████╗ ██████╗ ███╗   ██╗███████╗ ██████╗ ██╗     ███████╗      
                        ██╔════╝██╔═══██╗████╗  ██║██╔════╝██╔═══██╗██║     ██╔════╝      
                        ██║     ██║   ██║██╔██╗ ██║█████╗  ██║   ██║██║     █████╗        
                        ██║     ██║   ██║██║╚██╗██║██╔══╝  ██║   ██║██║     ██╔══╝        
                        ╚██████╗╚██████╔╝██║ ╚████║██║     ╚██████╔╝███████╗███████╗      
                         ╚═════╝ ╚═════╝ ╚═╝  ╚═══╝╚═╝      ╚═════╝ ╚══════╝╚══════╝      
                                                                                             
          ███████╗██╗   ██╗███████╗████████╗███████╗███╗   ███╗                              
          ██╔════╝╚██╗ ██╔╝██╔════╝╚══██╔══╝██╔════╝████╗ ████║                              
          ███████╗ ╚████╔╝ ███████╗   ██║   █████╗  ██╔████╔██║                              
          ╚════██║  ╚██╔╝  ╚════██║   ██║   ██╔══╝  ██║╚██╔╝██║                              
          ███████║   ██║   ███████║   ██║   ███████╗██║ ╚═╝ ██║                              
          ╚══════╝   ╚═╝   ╚══════╝   ╚═╝   ╚══════╝╚═╝     ╚═╝                              
                                                                                             
```

**把设计图变成代码，只需 3 步**

*AI 驱动的 UI 转代码工具 - 智能分析 · 自动生成 · 无缝集成*

[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](https://opensource.org/licenses/MIT)
[![Cursor](https://img.shields.io/badge/For-Cursor-blue.svg)](https://cursor.sh)

</div>

---

## ✨ 工作流程

```
┌─────────────────────────────────────────────────────────────────────────────────┐
│                                                                                 │
│     ┌─────────┐                                                                 │
│     │ 📸 设计图 │                                                                 │
│     │  (输入)   │                                                                 │
│     └────┬────┘                                                                 │
│          │                                                                        │
│          ▼                                                                        │
│   ┌─────────────────────────────────────────┐                                  │
│   │         🧠 AI 智能分析                   │                                  │
│   │  ┌─────────┐ ┌─────────┐ ┌─────────┐    │                                  │
│   │  │  代码UI  │ │  图片   │ │  抠图   │    │                                  │
│   │  │  ✅ 85%  │ │  🎨 12% │ │  ✂️ 3%  │    │                                  │
│   │  └─────────┘ └─────────┘ └─────────┘    │                                  │
│   └─────────────────────────────────────────┘                                  │
│          │                                                                        │
│          ▼                                                                        │
│     ┌─────────┐     ┌─────────┐     ┌─────────┐                               │
│     │  代码   │  +  │  图片   │  +  │  交互   │                               │
│     │  🚀     │     │  🎨     │     │  ⚡     │                               │
│     └─────────┘     └─────────┘     └─────────┘                               │
│          │                                                                        │
│          ▼                                                                        │
│     ┌─────────┐                                                                 │
│     │ 💯 完成  │                                                                 │
│     │ (输出)   │                                                                 │
│     └─────────┘                                                                 │
│                                                                                 │
└─────────────────────────────────────────────────────────────────────────────────┘
```

---

## 🎯 功能对比

| 场景 | 以前 | 现在 |
|:-----|:-----|:-----|
| 看到好看的设计图 | 😩 不知道从何下手 | 🚀 一键分析出实现方案 |
| 需要提取图片资产 | ⏰ 手动一个个导出 | ⚡ AI 自动识别生成 |
| 想做抠图透明背景 | 💰 用 Photoshop | ✂️ AI 智能抠图 |
| 验证还原度 | 👀 肉眼对比 | 🔍 自动标注差异 |

---

## 🔥 核心功能

### 1️⃣ 智能分析
```
输入: 设计截图 / Figma 导出图 / App UI 截图
      ↓
AI 分析:
  ├─ 🔧 可代码实现  →  按钮、卡片、导航、表单、布局
  ├─ 🎨 需要生成    →  插画、照片、背景、纹理
  └─ ✂️ 需要抠图    →  产品图、人物、物体透明背景
      ↓
输出: 详细的前期审查报告
```

### 2️⃣ 资产规划
自动生成包含以下信息的资产清单：
- 📐 **尺寸** - 目标槽位 & 导出尺寸
- 📝 **提示词** - 适合 AI 生图的描述
- 🎯 **用途** - UI 位置和集成方式
- 🔧 **后处理** - 裁剪、压缩、去背景需求

### 3️⃣ 代码生成
```
┌──────────────┐     ┌──────────────┐     ┌──────────────┐
│   HTML/CSS   │  +  │  React/Vue  │  +  │  组件化代码  │
│   基础结构    │     │  现代框架    │     │  可复用     │
└──────────────┘     └──────────────┘     └──────────────┘
```

### 4️⃣ 质量审查
```
原图对比 ──→ 差异分析 ──→ 差异标注 ──→ 调整优化 ──→ 最终验收
  🔍           📊          🏷️          ✏️           ✅
```

---

## 👥 适用人群

<div align="center">

| 🎨 设计师 | 💻 开发者 | 📱 产品经理 | 🚀 创业者 |
|:---------:|:---------:|:-----------:|:---------:|
| 验证设计能否实现 | 快速拿到可运行代码 | 快速搭建原型 | 把灵感变成产品 |
| 获取开发建议 | 专注业务逻辑 | 可交互原型演示 | 0→1 快速验证 |

</div>

---

## 🚀 快速开始

### 在 Cursor 中使用

```markdown
1️⃣  安装 Skill 到你的 Cursor 项目
2️⃣  上传 UI 参考图
3️⃣  发送指令：
```

> 💬 **"帮我分析这个设计稿"**
>
> 💬 **"把这个 UI 转成代码"**
>
> 💬 **"提取页面中的图片资产"**

---

## 📖 完整工作流

### Step 1: 上传参考图
```
支持格式: PNG / JPG / WebP / Figma 截图 / App 截图
```

### Step 2: 获取分析报告
```
📋 前期审查文档

整体判断:
├── 页面类型: 电商 App 首页
├── 视觉风格: 简约现代 + 渐变卡片
└── 实现风险: 中等

元素分类:
├── ✅ 容易实现 (75%)
│   └── 导航栏、搜索框、商品卡片、按钮
├── 🎨 需要生成 (20%)
│   └── 首屏 Banner、分类图标、空状态插画
└── ✂️ 需要抠图 (5%)
    └── 商品白底图、模特图

需要确认:
├── ❓ Logo 是否使用原图？
├── ❓ 字体是否接受近似替代？
└── ❓ 移动端是否需要单独适配？
```

### Step 3: 确认后生成
```
⏳ 正在生成...
├── 🎨 生成首屏背景图... ✓
├── 🎨 生成空状态插画... ✓
├── ✂️ 抠取商品透明图... ✓
├── 💻 生成 React 代码... ✓
└── 🔍 验证还原度... ✓

✅ 完成！生成 5 个资产 + 1 套可运行代码
```

### Step 4: 审查验收
```
还原度检查报告:
├── 布局比例        ✅ 95% 匹配
├── 字体还原        ✅ 90% 相似
├── 图片融合        ✅ 视觉一致
├── 响应式适配      ✅ 桌面/移动端正常
└── 交互跳转        ✅ 可点击元素完整

🎉 可以发布到生产环境了！
```

---

## ⚡ 效果对比

<div align="center">

| 指标 | 传统方式 | 使用本 Skill |
|:-----|:---------|:-------------|
| 分析时间 | 30 分钟 | **3 分钟** |
| 提取资产数量 | 手动提取 | **自动识别** |
| 代码可用性 | 需要调整 | **直接可用** |
| 还原度验证 | 肉眼对比 | **自动标注** |

</div>

---

## 🎨 支持的输出

### 代码框架
```
✅ HTML / CSS
✅ React
✅ Vue
✅ Tailwind CSS
✅ 响应式布局
```

### 图片格式
```
✅ PNG (透明背景)
✅ WebP (压缩优化)
✅ JPG (照片)
✅ 支持 2x 视网膜屏
```

### 智能识别
```
✅ 桌面端 UI
✅ 移动端 App
✅ 响应式设计
✅ 深色/浅色模式
```

---

## 🔒 注意事项

| ⚠️ 建议 | 💡 说明 |
|:--------|:--------|
| Logo 和商标 | 使用用户提供的原图，不要 AI 重画 |
| 特殊字体 | 需要确认授权，优先使用开源字体 |
| 版权素材 | 人物照片、真实产品建议用原图 |
| 生成风格 | AI 生成可能略有差异，可多次尝试 |

---

## 📚 相关资源

| 文件 | 说明 |
|:-----|:-----|
| 📄 [SKILL.md](./SKILL.md) | 完整 Skill 规范文档 |
| 🤖 [agents](./agents/) | Agent 配置文件 |
| 📖 [references](./references/) | 参考文档和模板 |

---

## 🤝 贡献

欢迎 Star ⭐ | Issues 🐛 | PRs 🔀

---

<div align="center">

**让设计到代码的转化，像魔法一样简单 ✨**

*Made with ❤️ for developers and designers*

</div>
